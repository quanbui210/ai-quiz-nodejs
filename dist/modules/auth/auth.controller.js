  ...
  try {
    ...
    if (error || !data?.url) {
      return res.status(400).json({
        error: error?.message || "No redirect URL received" || "OAuth error",
      });
    }
    return res.json({
      url: data.url,
  ...
  try {
    if (req.method === "POST" && req.body.access_token) {
      ...
      if (!user) {
        ...
        try {
          const { getOrCreateDefaultSubscription } =
    ...
    if (code) {
      const { data, error } =
        await supabase_1.supabase.auth.exchangeCodeForSession(code);
      if (error) {
