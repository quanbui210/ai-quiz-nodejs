...
function parseQuizResponse(quizText) {
    try {
        ...
        const questions = questionBlocks
            ...
            const correctMatchesOption = validOptions.some((opt) => opt.toLowerCase().trim() === normalizedCorrect.toLowerCase().trim() ||
...
const createQuiz = async (req, res) => {
    try {
        ...
        const invalidQuestions = parsedQuiz.questions.filter((q) => q.options.length !== 4 || q.options.some(opt => !opt || opt.trim().length === 0));
        ...
        const questionsWithInvalidAnswers = parsedQuiz.questions
            ...
            if (!exactMatch && partialMatch) {
                const matchedOption = q.options.find(opt => opt.trim().toLowerCase().replace(/\s+/g, ' ').includes(normalizedCorrect) ||
...
const resumeQuiz = async (req, res) => {
    try {
        ...
        const savedAnswersMap = new Map(attempt.answers.map((a) => [a.questionId, a.userAnswer]));
        return res.json({
            attemptId: attempt.id,
            quizId: quiz.id,
            quizTitle: quiz.title,
