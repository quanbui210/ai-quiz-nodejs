...
exports.deleteDocument = exports.getDocument = exports.listDocuments = exports.uploadDocument = void 0;
...
const TEMP_DIR = path_1.default.join(os_1.default.tmpdir(), "document-processing");
...
const uploadDocument = async (req, res) => {
    try {
        ...
        const { getUserUsage } = await Promise.resolve().then(() => __importStar(require("../../utils/usage")));
        const usage = await getUserUsage(req.user.id);
        const subscription = await prisma_1.default.userSubscription.findUnique({
            where: { userId: req.user.id },
            include: { plan: true },
        });
        const maxDocuments = subscription?.maxDocuments || subscription?.plan?.maxDocuments || 0;
        ...
        processDocumentAsync(document.id, storagePath, file.mimetype).catch((error) => {
...
async function processDocumentAsync(documentId, storagePath, mimeType) {
    const tempFilePath = path_1.default.join(TEMP_DIR, `${documentId}-${Date.now()}${path_1.default.extname(storagePath)}`);
    try {
        ...
        const processed = await (0, document_processor_1.processDocument)(tempFilePath, mimeType, {
            chunkSize: 1000,
            chunkOverlap: 200,
        });
        ...
        const embeddings = await (0, embeddings_1.generateEmbeddingsBatch)(texts, 100);
        const embeddingData = processed.chunks
            ...
            .filter((item) => item.embedding !== undefined && item.embedding.length > 0);
        ...
        console.log(`Document ${documentId} processed successfully: ${processed.chunks.length} chunks`);
    }
    catch (error) {
    ...
    try {
        ...
        if (document.filePath) {
            try {
                ...
                console.error(`Failed to delete file from storage ${document.filePath}:`, error);
            }
        }
        if (id) {
