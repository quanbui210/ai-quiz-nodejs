...
exports.generateQuizFromDocument = exports.getDocumentQuizzes = void 0;
...
const getDocumentQuizzes = async (req, res) => {
    try {
        if (!req.user?.id) {
...
exports.getDocumentQuizzes = getDocumentQuizzes;
const generateQuizFromDocument = async (req, res) => {
    try {
        ...
        const { documentId } = req.params;
        const { topicId, title, difficulty = "INTERMEDIATE", questionCount = 10, quizType = "MULTIPLE_CHOICE", timer, focus, } = req.body;
        const document = await prisma_1.default.document.findFirst({
            where: {
                id: documentId,
                userId: req.user.id,
        ...
        let relevantChunks = [];
        try {
            const embeddingCount = await prisma_1.default.documentEmbedding.count({
            ...
            console.log(`Searching for similar chunks in document ${documentId} (${embeddingCount} embeddings available)`);
            let chunks = [];
            let similarityThreshold = 0.6;
            chunks = await (0, pgvector_1.findSimilarChunks)(documentId, queryEmbedding, Math.min(questionCount * 2, 20), similarityThreshold);
            ...
            documentContext = chunks
                .map((chunk, index) => `[Section ${index + 1}]: ${chunk.chunkText.substring(0, 800)}`)
                .join("\n\n");
        }
        catch (error) {
...
10. Do NOT add any closing text after the last question`;
        ...
        const createdQuiz = await prisma_1.default.quiz.create({
            data: {
                title: quizTitle,
                type: quizType,
                difficulty: difficulty,
                count: parsedQuiz.questions.length,
                status: client_1.QuizStatus.ACTIVE,
                timer: timer || null,
    ...
    try {
        ...
        for (const block of questionBlocks) {
            ...
            while (i < lines.length) {
                ...
                const correctMatch = line.match(/✅\s*Correct answer:\s*([A-D])\)\s*(.+)$/i);
...
//# sourceMappingURL=document-quiz.controller.js.map