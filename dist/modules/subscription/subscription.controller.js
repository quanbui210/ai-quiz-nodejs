    ...
    const plansWithPricing = await Promise.all(
      plans.map(async (plan) => {
        ...
        if (plan.stripePriceId) {
          try {
            const stripePrice = await stripe_1.stripe.prices.retrieve(
              plan.stripePriceId,
              {
                expand: ["product"],
              },
            ...
            if (productData) {
              if (typeof productData === "string") {
                try {
                  const fullProduct =
                    await stripe_1.stripe.products.retrieve(productData);
                  product = {
                    id: fullProduct.id,
...
const getMySubscription = async (req, res) => {
  try {
    ...
    if (!subscription) {
      subscription = await (0, subscription_1.getOrCreateDefaultSubscription)(
        req.user.id,
      );
    }
...
const createCheckoutSession = async (req, res) => {
  try {
    ...
    if (
      ...
      try {
        const portalSession =
          await stripe_1.stripe.billingPortal.sessions.create({
...
async function handleCheckoutCompleted(session) {
  ...
  const subscription = await stripe_1.stripe.subscriptions.retrieve(
...
async function handleSubscriptionUpdated(subscription) {
  ...
  try {
    latestSubscription = await stripe_1.stripe.subscriptions.retrieve(
      subscription.id,
      {
  ...
  if (newPlanId !== userSubscription.planId) {
    ...
    await (0, subscription_1.updateSubscriptionFromPlan)(
...
async function handlePaymentSucceeded(invoice) {
  ...
  const subscription =
    await stripe_1.stripe.subscriptions.retrieve(subscriptionId);
  await handleSubscriptionUpdated(subscription);
}
...
const cancelSubscription = async (req, res) => {
  try {
    ...
    await stripe_1.stripe.subscriptions.update(
      subscription.stripeSubscriptionId,
      {
