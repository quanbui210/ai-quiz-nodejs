-- Job Matching System Schema Migration
-- This replaces the old MarketInsight approach with a job matching system
...
-- Create unique index on externalId (only if column exists and index doesn't)
DO $$
BEGIN
  IF EXISTS (
...
-- Create unique index on jobId (if not exists)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
...
CREATE UNIQUE INDEX IF NOT EXISTS "market_trends_unique" 
ON "MarketTrends"("country", "location", "role", "periodStart");

-- Create indexes for MarketTrends
...
-- Create vector index for Resume CV embeddings (if column exists)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    ...
    CREATE INDEX IF NOT EXISTS "Resume_cvEmbedding_idx" 
    ON "Resume" 
    USING hnsw ("cvEmbedding" vector_cosine_ops);
  END IF;
END $$;

-- Add index on Resume extractedSkills
